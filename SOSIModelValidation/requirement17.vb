Partial Public Class ModelValidation
    ...
    Sub requirement17(thePackageElement As EA.Element)
            ...
            If connector.Type = "Usage" Or connector.Type = "Package" Or connector.Type = "Dependency" Then
                If thePackageElement.ElementID = connector.ClientID Then
                    If Not packageDependenciesShownElementIDList.Contains(connector.SupplierID) Then
                        ...
                        Select Case ruleSet
                            Case "SOSI"
                                Output("Error: Dependency to package [«" & dependee.Stereotype & "» " & dependee.Name & "] exist in model, but is not shown in any package diagram [/krav/17]")
                            Case "19109", "19103"
                                Output("Error: Dependency to package [«" & dependee.Stereotype & "» " & dependee.Name & "] exist in model, but is not shown in any package diagram [ISO19103:2015/requirement17]")
                        End Select
                        errorCounter += 1
                    End If
                End If
        ...
        For Each externalElementID In externalReferencedElementIDList
            ...
            If Not PackageIsShown(externalElementPackage) Then
                Select Case ruleSet
                    Case "SOSI"
                        Output("Error:  Dependency to package [«" & externalElementPackage.Element.Stereotype & "» " & externalElementPackage.Name & "] (or any of its superpackages) containing element [«" & externalElement.Stereotype & "» " & externalElement.Name & "] is not shown in any package diagram [/krav/17]")
                    Case "19109", "19103"
                        Output("Error:  Dependency to package [«" & externalElementPackage.Element.Stereotype & "» " & externalElementPackage.Name & "] (or any of its superpackages) containing element [«" & externalElement.Stereotype & "» " & externalElement.Name & "] is not shown in any package diagram [ISO19103:2015/requirement17]")
                End Select
                errorCounter += 1
            End If
        Next
